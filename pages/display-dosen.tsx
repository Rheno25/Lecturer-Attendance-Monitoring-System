//pages/display-dosen.tsx

import { NextPage } from "next";
import { useRouter } from "next/router";
import { useCallback, useState, useEffect } from "react";
import FrameComponent2 from "../components/atoms/frame-component4";
import styles from "./index.module.css";
import FrameComponent, { FrameComponentType } from "../components/atoms/frame-component5";
import axios from "axios";

const DisplayDosen: NextPage = () => {
    const router = useRouter();
    const { username } = router.query;
    const usernameString = typeof username === 'string' ? username : ''; // Ensure username is a string
    ...
    return (
        <div className="w-full relative bg-lightblue overflow-hidden flex flex-col items-start justify-start gap-[73px] tracking-[normal] mq450:gap-[18px_73px] mq750:gap-[36px_73px]">
            <FrameComponent2 onImage2IconClick={onImage2IconClick} username={usernameString} />
                        ...
                        {`Lecturer Detail: `}
                            {lecturerDetail && !lecturerInputVisible && (
                                <div className="w-full text-left py-0 px-0 inline-block z-[1] mq450:text-lgi mq750:text-7xl">
                                    ...
                                    <center>
                                        <button
                                            onClick={() => setLecturerInputVisible(true)}
                                            className="ml-2 p-1 bg-yellow-500 text-white rounded"
                                        >
                                            Update
                                        </button>
                                        <button
                                            onClick={() => setLecturerDetail('')}
                            ...
                            {lecturerInputVisible && (
                                <div className="mt-2">
                                    <input
                                        type="text"
                                        className="mt-2 w-full border border-gray-300 p-2 rounded mq450:text-base"
                                        value={lecturerDetail}
                                        onChange={(e) => setLecturerDetail(e.target.value)}
                                        placeholder="Enter lecturer detail"
                                    />
                                    <button
